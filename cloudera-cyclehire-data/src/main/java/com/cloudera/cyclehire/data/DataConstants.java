...
public class DataConstants {
  ...
  public static final String[] PATH_LOCAL_TARS = new File(PATH_LOCAL_TAR).list(new FilenameFilter() {
    @Override
  ...
  public static final String[] PATH_LOCAL_XMLS = new File(PATH_LOCAL_XML).list(new FilenameFilter() {
    @Override
    public boolean accept(File dir, String name) {
  ...
  static {
    for (int i = 0; i < PATH_LOCAL_XMLS.length; i++) {
      PATH_LOCAL_XMLSS[i] = new File(PATH_LOCAL_XML + "/" + PATH_LOCAL_XMLS[i]).list(new FilenameFilter() {
        @Override
        public boolean accept(File dir, String name) {
          return name.endsWith(".xml");
        }
