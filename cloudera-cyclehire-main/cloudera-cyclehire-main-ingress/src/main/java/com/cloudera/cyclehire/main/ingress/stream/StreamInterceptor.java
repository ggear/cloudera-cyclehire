...
public class StreamInterceptor implements Interceptor {
  ...
  private static final Logger LOG = LoggerFactory.getLogger(StreamInterceptor.class);

  @Override
  public void initialize() {
    if (LOG.isInfoEnabled()) {
      LOG.info("Stream Interceptor initialised, Agent ID [" + StreamEvent.AGENT_ID + "]");
    }
  }
  ...
  private String getEventBatchHeader(Event first, Event last, long timestamp) {
    return putHeader(first, StreamEvent.HEADER_TIMESTAMP, "" + timestamp / 1000) + "_"
  ...
  private Event getEventWithHeaders(Event event, String batch, int index, int total, long timestamp) {
    ...
    putHeader(event, StreamEvent.HEADER_TYPE, StreamEvent.Type.POLL.toString().toLowerCase());
    putHeader(event, StreamEvent.HEADER_TIMESTAMP, "" + timestamp / 1000);
    putHeader(event, StreamEvent.HEADER_BATCH, batch, true);
    putHeader(event, StreamEvent.HEADER_INDEX, String.format("%03d", index), true);
    putHeader(event, StreamEvent.HEADER_TOTAL, String.format("%03d", total), true);
    return event;
  }
